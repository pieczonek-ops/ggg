...
export function ArticleCard({ article, featured, onClick }: ArticleCardProps) {
  ...
  return (
    <motion.div
      whileHover={{ y: -8 }}
      onClick={onClick}
      className={`group cursor-pointer bg-zinc-900/40 border border-zinc-800/50 rounded-[2rem] overflow-hidden hover:border-emerald-500/50 transition-all duration-500 flex flex-col ${
    ...
    >
      ...
      <div className={`p-8 flex flex-col justify-between ${featured ? 'md:w-1/2' : ''}`}>
        ...
        <div className="flex items-center text-emerald-500 font-bold text-sm group/btn">
          Czytaj więcej 
